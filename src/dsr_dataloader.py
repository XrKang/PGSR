...
class Train_RGBDD_real(Dataset):
    ...
    def __getitem__(self, index):
        ...
        rgb = (rgb - np.array([0.485, 0.456, 0.406]).reshape(3, 1, 1)) / np.array([0.229, 0.224, 0.225]).reshape(3, 1,
                                                                                                                 1)
...
class Valid_RGBDD_real(Dataset):
    ...
    def __getitem__(self, index):
        ...
        return depth_lr, rgb, depth_hr, depth_min, depth_max, self.depth_names[index][:-4]

class Train_RGBDD_real_noise(Dataset):
    def __init__(self, args):
        super(Train_RGBDD_real_noise, self).__init__()
        ...
        self.data_augmentation = args.augmentation
        self.crop_patch = args.crop_patch
        self.noise_levle = args.noise_level
    ...
    def __getitem__(self, index):
        ...
        h, w = rgb.shape[1:]
        depth_lr = np.array(Image.fromarray(depth_lr).resize((w, h), Image.BICUBIC))
        noise = np.random.normal(0, self.noise_levle/255 * depth_lr.max(), depth_lr.shape)
        depth_lr = depth_lr + noise
...
class Valid_RGBDD_real_noise(Dataset):
    def __init__(self, args):
        super(Valid_RGBDD_real_noise, self).__init__()
        self.depth_hr_path = args.depth_hr_path_valid
        self.depth_lr_path = args.depth_lr_path_valid
        self.depth_names = os.listdir(self.depth_hr_path)
        self.noise_levle = args.noise_level
...
if __name__ == '__main__':
    ...
    # for i, (depth_lr, rgb, depth_hr) in enumerate(training_data_loader):
    for i, (depth_lr, rgb, depth_hr) in enumerate(valid_data_loader):
